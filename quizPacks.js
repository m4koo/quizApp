let htmlQuestions = [
    {
        "question": "Für was steht die Abkürzung 'HTML'?",
        "answer_1": "How To Make Lasagna",
        "answer_2": "Hyper Text Markup Language",
        "answer_3": "Hey Too Much Layout",
        "answer_4": "Hieroglyphics, To Machine Legible",
        "correct_answer": 2
    },
    {
        "question": "Wer ist für die Web standards zuständig?",
        "answer_1": "Mozilla",
        "answer_2": "The World Wide Web Consortium (W3C)",
        "answer_3": "Google",
        "answer_4": "Microsoft",
        "correct_answer": 2
    },
    {
        "question": "Mit welchem HTML Element erstellt man eine numerierte Liste?",
        "answer_1": '<nl>',
        "answer_2": '<ul>',
        "answer_3": '<list>',
        "answer_4": '<ol>',
        "correct_answer": 4
    },
    {
        "question": "Wie öffnet man einen Link in einem neuen Tab?",
        "answer_1": "<a href='url' target='_blank'>",
        "answer_2": "<a href='url' target='new'>",
        "answer_3": "<a href='url' target='new_tab'>",
        "answer_4": "<a href='url' new>",
        "correct_answer": 1
    },
    {
        "question": "Wie heißt der Erfinder von HTML?",
        "answer_1": "Tim Berners-Lee",
        "answer_2": "Jeff Bezoz",
        "answer_3": "Brendan Eich",
        "answer_4": "Bill Gates",
        "correct_answer": 1
    },
];


let cssQuestions = [
    {
        "question": "Für was steht die Abkürzung 'CSS'?",
        "answer_1": "Counter-Strike: Source",
        "answer_2": "Common Styling Stuff",
        "answer_3": "Cascading Style Sheets",
        "answer_4": "Cool Super-Saiyajin",
        "correct_answer": 3
    },
    {
        "question": "Wie bindet man eine CSS-Datei korrekt in HTML ein?",
        "answer_1": "<link rel='stylesheet' href='style.css'>",
        "answer_2": "<stylesheet>style.css</stylesheet>",
        "answer_3": "<style src ='style.css'>",
        "answer_4": "<script src='style.css'>",
        "correct_answer": 1
    },
    {
        "question": "Wie kommentiert man in einer CSS-Datei?",
        "answer_1": '// Kommentar',
        "answer_2": '/* Kommentar */',
        "answer_3": '<!-- Kommentar -->',
        "answer_4": '// Kommentar //',
        "correct_answer": 2
    },
    {
        "question": "Wie wählt man alle p Elemente in einem div aus?",
        "answer_1": "div p",
        "answer_2": "div+p",
        "answer_3": "div.p",
        "answer_4": "div:all(p)",
        "correct_answer": 1
    },
    {
        "question": "Welches ist der Standardwert von 'position'?",
        "answer_1": "relative",
        "answer_2": "absolute",
        "answer_3": "sticky",
        "answer_4": "static",
        "correct_answer": 4
    },
    {
        "question": "Welche CSS Eigenschaft ändert die Text Größe?",
        "answer_1": "font-weight",
        "answer_2": "font-size",
        "answer_3": "size",
        "answer_4": "text-size",
        "correct_answer": 2
    }
];


let jsQuestions = [
    {
        "question": "Was is Javascript?",
        "answer_1": "Java",
        "answer_2": "Programmiersprache",
        "answer_3": "Horror",
        "answer_4": "Copy Paste",
        "correct_answer": 2,
    },
    {
        "question": "Wie erstellt man eine Funktion?",
        "answer_1": "function myFunction()",
        "answer_2": "new function = myFunction()",
        "answer_3": "function = myFunction()",
        "answer_4": "myFunction()",
        "correct_answer": 1,
    },
    {
        "question": "Wie sieht eine IF Bedingung aus?",
        "answer_1": 'if x == y',
        "answer_2": 'if x == y then',
        "answer_3": 'if x = y',
        "answer_4": 'if (x == y)',
        "correct_answer": 4,
    },
    {
        "question": "Welches ist ein korrektes Array?",
        "answer_1": "let arr = {1:red, 2:green, 3:blue}",
        "answer_2": "arr = red, green, blue",
        "answer_3": "let arr = ['red', 'green', 'blue']",
        "answer_4": "let arr = array('red', 'green', 'blue')",
        "correct_answer": 3,
    }
];

